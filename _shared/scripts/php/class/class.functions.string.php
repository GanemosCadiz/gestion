...
class string {
	 ...
	 * @return
	 */
	public static function toParam($string) {
	...
	public static function filenameSuffix($file, $suffix) {
		...
		$extension = (strpos($file, ".") === false) ? "" : strtolower(substr(strrchr($file, "."), 1));
		$filename = (strpos($file, ".") === false) ? $file : substr($file, 0, strrpos($file, "."));
	 ...
	 * @return
	 */
	public static function passwordGet($length = 10, $add_dashes = false, $available_sets = 'luds') {
	 ...
	 * @return
	 */
	public static function normalize($string, $strict=false) {
		$raw_string = $string;
		$string = string::stripAllTags($string);
		$string = string::removeAccents($string);
		// Kill octets
	 ...
	 * @return
	 */
	public static function normalizeUrl($string) {
		$string = string::normalize($string);
		if (string::seemsUTF8($string)) {
	...
	/**
	 * string::UTF8URIEncode()
	 * 
	 * @param mixed $utf8_string
	 * @param integer $length
	 * @return
	 */
